...
class MyHomeClient {
    ...
    onMonitor(_frame) {
        ...
        frames.forEach(function(frame) {
            ...
            extract = frame.match(/^\*#2\*([0-9#]+)\*10\*10\*(\d+)\*\d+\*\d+$/);                 
            if (extract) {
                var address = this._addressToSlashes(extract[1]);
                var position = parseInt(extract[2],10);
                debug(sprintf("ADVANCEDBLIND %s ADDRESS %s",position,address));
                if (this.parent != null && this.parent.onMonitor != null && typeof(this.parent.onSimpleBlind) == 'function') this.parent.onAdvancedBlind(address,position);
            }
