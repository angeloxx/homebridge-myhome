...
class MyHomeClient {
    ...
    onMonitor(_frame) {
        ...
        frames.forEach(function(frame) {
            ...
            if (extract) {
                let value = false;
                if (parseInt(extract[1],10) == 3)
                  value = true;
                if (parseInt(extract[1],10) == 4)
                  value = false;
                let address = parseInt(extract[2],10);
                if (this.parent != null && this.parent.onMonitor != null && typeof(this.parent.onScenario) == 'function')
                	this.parent.onScenario(address,value);
